...
class CheolFingersEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        self.update_threshold = np.linalg.norm(np.ones((1,n_actions))*3e-6)
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        return -(est_dist > self.distance_threshold).astype(np.float32) - 0.4*force_rew - 1.*vel_rew #- 1.*flag_rew
    ...
    def _sample_goal(self):
        ...
        if self.eval_env == False:
            ...
            self.sim.model.geom_size[self.sim.model.geom_name2id('object_top'),1] = 0.01 + (np.random.random_sample())*0.025
