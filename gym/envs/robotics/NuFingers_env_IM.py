...
def pos_ctrl_fnc(obs):
    load_path = "C:/Users/mk37972/Coding/models/NuFingers/harsh_65/For_IM/Sim_NuFingers_bad2dim_10"
    ...
    input_var = np.matmul(loaded_params['ddpg/main/pi/_0/kernel:0'].transpose(), input_var.reshape([20,1]))
    ...
    input_var = loaded_params['ddpg/main/pi/_3/bias:0'].reshape([2,1]) + input_var.reshape([2,1])
...
class NuFingersEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        self.glob_obs = {
                ...
                'achieved_goal': np.zeros([1,7]),
                'desired_goal': np.zeros([1,7]),
            }
    ...
    def compute_reward(self, achieved_goal, goal, info):
        try: 
            ...
            fragile_goal = 1 * np.linalg.norm(achieved_goal[:,self.goal_dim : self.goal_dim + self.velgoal_dim] - goal[:,self.goal_dim : self.goal_dim + self.velgoal_dim], axis=-1) + 4.0 * np.linalg.norm((achieved_goal[:,self.goal_dim + self.velgoal_dim:] - goal[:,self.goal_dim + self.velgoal_dim:])*((achieved_goal[:,self.goal_dim + self.velgoal_dim:] - goal[:,self.goal_dim + self.velgoal_dim:]) < 0), axis=-1)
        except: 
            d = goal_distance(achieved_goal[:self.goal_dim], goal[:self.goal_dim])
            fragile_goal = 1 * np.linalg.norm(achieved_goal[self.goal_dim : self.goal_dim + self.velgoal_dim] - goal[self.goal_dim : self.goal_dim + self.velgoal_dim], axis=-1) + 4.0 * np.linalg.norm((achieved_goal[self.goal_dim + self.velgoal_dim:] - goal[self.goal_dim + self.velgoal_dim:])*((achieved_goal[self.goal_dim + self.velgoal_dim:] - goal[self.goal_dim + self.velgoal_dim:]) < 0), axis=-1)
    ...
    def _set_action(self, action):
        ...
        action = np.concatenate([pos_ctrl_fnc(self.glob_obs), action])
    ...
    def _get_obs(self):
        ...
        # elif np.linalg.norm(self.goal[0] - self.sim.data.qpos[self.sim.model.jnt_qposadr[self.sim.model.joint_name2id('Sensor_joint')]]) < self.distance_threshold:
        #     self.sim.model.geom_rgba[-3][1:3] = 0.6
        #     self.sim.model.geom_rgba[-2][1:3] = 0.6
        # else:
