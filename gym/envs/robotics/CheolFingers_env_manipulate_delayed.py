...
class CheolFingersEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        self.actual_max_stiffness = 2.30399e-1*10.
        self.actual_max_stiffness2 = 1.15199e-1*10.
        self.actual_max_friction = 1.e-2*10.
        self.actual_max_friction2 = 1.e-2*10.
    ...
    def compute_reward(self, achieved_goal, goal, info):
        try: 
            ...
            force_rew = np.linalg.norm((achieved_goal[:,self.est_dim:self.est_dim+self.force_dim] - goal[:,self.est_dim:self.est_dim+self.force_dim]) * (achieved_goal[:,self.est_dim:self.est_dim+self.force_dim] < 0), axis=-1)
        ...
        return -(est_dist > self.distance_threshold).astype(np.float32) - 0.1*force_rew - 1.*vel_rew #- 1.*flag_rew
    ...
    def _set_action(self, action):
        ...
        r = np.array([[self.prev_stiffness],[0.8],[0.8]])
    ...
    def _get_obs(self):
        ...
        if self.update_bool == True: 
            ...
            xR = L1 * np.cos(unobserved_Rj[0,0] + np.pi/4.0) + L2 * np.cos(unobserved_Rj[0,0]-unobserved_Rj[1,0] + np.pi/4.0)
            yR = L1 * np.sin(unobserved_Rj[0,0] + np.pi/4.0) + L2 * np.sin(unobserved_Rj[0,0]-unobserved_Rj[1,0] + np.pi/4.0)
            xL = L1 * np.cos(unobserved_Lj[0,0] + 3*np.pi/4.0) + L2 * np.cos(unobserved_Lj[0,0]+unobserved_Lj[1,0] + 3*np.pi/4.0)
            yL = L1 * np.sin(unobserved_Lj[0,0] + 3*np.pi/4.0) + L2 * np.sin(unobserved_Lj[0,0]+unobserved_Lj[1,0] + 3*np.pi/4.0)
            
            P_R = np.array([xR, yR])
            P_L = np.array([xL, yL])
            Prel = P_R - P_L + np.array([0.127, 0])
            l = np.sqrt(Prel[0]*Prel[0] + Prel[1]*Prel[1])
            p = np.array([[l],[(xR+xL+0.127)/2],[(yR+yL)/2]])
            
            Jp = np.matrix([[-Prel[0]/p[0,0], -Prel[1]/p[0,0], Prel[0]/p[0,0], Prel[1]/p[0,0]],
                          [0.5, 0, 0.5, 0],
                          [0, 0.5, 0, 0.5]])
            a, b = Jp[0,2], Jp[0,3]
            c = a**2 + b**2
            ...
            J = np.matrix([[-P_L[1], -L2 * np.sin(unobserved_Lj[0,0]+unobserved_Lj[1,0] + 3*np.pi/4.0), 0, 0], 
                             [P_L[0], L2 * np.cos(unobserved_Lj[0,0]+unobserved_Lj[1,0] + 3*np.pi/4.0), 0, 0],
                             [0, 0, -P_R[1], -L2 * np.sin(unobserved_Rj[0,0]-unobserved_Rj[1,0] + np.pi/4.0)], 
                             [0, 0, P_R[0], L2 * np.cos(unobserved_Rj[0,0]-unobserved_Rj[1,0] + np.pi/4.0)]])
            J_inv_L = np.matrix([[J[1,1], -J[0,1]], [-J[1,0], J[0,0]]]) / (J[0,0]*J[1,1] - J[0,1]*J[1,0])
            J_inv_R = np.matrix([[J[3,3], -J[2,3]], [-J[3,2], J[2,2]]]) / (J[2,2]*J[3,3] - J[2,3]*J[3,2])
            ...
            # desired task space force
            r = np.array([[self.prev_stiffness],[0.8],[0.8]])
            self.des_Fp =  max_kp_diag * (r * (self.des_p - p))
        ...
        grasping_force = self.prev_force + (self.des_Fp[0,0] - self.prev_force) * dt / 0.5
        ...
        if self.max_grasping_force > grasping_force: 
            self.max_grasping_force = grasping_force
        ...
        self.prev_force = grasping_force
        
        # Is object within grasp?
        if self.eval_env == True:
            if grasping_force< -0.1 and np.linalg.norm(self.vel_p[0,0]) < 1e-2 and self.p[0,0] > 0.025 and self.p[0,0] < 0.12:
            ...
            if self.p[0,0] > 0.025 and self.p[0,0] < 0.12 and object_frc > self.min_grip and l_finger_force > 0.0 and r_finger_force > 0.0:# and l_finger_force * r_finger_force == 0.):
        ...
        modified_obs = dict(observation=observation, achieved_goal=np.array([object_pos_frame[0,0], object_pos_frame[1,0], object_pos_frame[2,0], self.max_grasping_force, self.max_vel_L, self.max_vel_R]), desired_goal = self.goal)
        return modified_obs
    ...
    def _reset_sim(self):
        ...
        self.sim.model.geom_size[self.sim.model.geom_name2id('object_bottom'),2] = 0.05
        self.sim.model.body_mass[self.sim.model.body_name2id('object')] = 1e-4
    ...
    def _sample_goal(self):
        goal = np.array([np.random.random_sample()*0.07+0.02, np.random.random_sample()*0.03+0.09, 0.05, 0.0, 0.0, 0.0])
        
        initial_qpos = self.sim.data.get_joint_qpos('object:joint').copy()
        initial_pos, initial_quat = initial_qpos[:3], initial_qpos[3:]
        initial_pos[:2] = np.array([-(np.random.random_sample()*0.02 + 0.08) + 0.1, np.random.random_sample()*0.07+0.02 - 0.0635])
