...
class CheolFingersEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        try: 
            ...
            force_rew = np.linalg.norm((achieved_goal[:,self.est_dim:] - goal[:,self.est_dim:])*(achieved_goal[:,self.est_dim:] - goal[:,self.est_dim:] < 0), axis=-1)
        except: 
            est_dist = goal_distance(achieved_goal[:self.est_dim], goal[:self.est_dim])
            force_rew = np.linalg.norm((achieved_goal[self.est_dim:] - goal[self.est_dim:])*(achieved_goal[self.est_dim:] - goal[self.est_dim:] < 0), axis=-1)
        ...
        return -(est_dist > self.distance_threshold).astype(np.float32) - 1. * force_rew
    ...
    def _set_action(self, action):
        ...
        self.est_grasping_force = des_Fp[0,0]
    ...
    def _get_obs(self):
        ...
        object_pos = np.array([self.sim.data.get_body_xpos('object')[1]+0.0635, -(self.sim.data.get_body_xpos('object')[0]-0.0873)])
        ...
        obj_acc = self.sim.data.qacc[self.sim.model.joint_name2id('object3:joint')] * 100
        
        if self.n_actions == 5:
            observation = np.array([self.p[0,0], self.p[1,0], self.p[2,0], # l, theta, cen_x, cen_y
                                    self.vel_p[0,0],
                                    self.goal[0]-object_pos[0], self.goal[1]-object_pos[1],
                                    object_pos[0]-self.p[1,0], object_pos[1]-self.p[2,0],
                                    self.est_grasping_force, 
        ...
        if self.est_grasping_force < -2e-1 and np.linalg.norm(self.est_obj_pose - self.p[1:3]) < 0.05 and np.linalg.norm(self.vel_p[0,0]) < 0.001 and self.p[0,0] > 0.03:
        ...
        modified_obs = dict(observation=observation, achieved_goal=np.array([object_pos[0], object_pos[1], self.est_grasping_force, obj_acc]), desired_goal = self.goal)
    ...
    def _sample_goal(self):
        goal = np.array([np.random.random_sample()*0.07+0.02, np.random.random_sample()*0.04+0.09, 0.0, 0.0])
