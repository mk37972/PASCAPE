...
def pos_ctrl_fnc(obs):
    load_path = "C:/Users/mk37972/Coding/models/block/harsh_65/For_IM/fpp_demo25bad4dim_10"
...
def pos_ctrl_fnc2(obs):
    load_path = "C:/Users/mk37972/Coding/models/chip/harsh_85/For_IM/fpp_demo25bad3dim_10"
    loaded_params = joblib.load(os.path.expanduser(load_path))
...
class FetchEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        self.object_fragility = 300.0 if self.model_path.find('wall') == -1 else 200.0
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if self.fragile_on:
            ...
            if self.reward_type == 'sparse':
                reward = -(d > self.distance_threshold).astype(np.float32) - np.float32(fragile_goal) * 2# - np.float32(penalty)/50.0
    ...
    def _step_callback(self):
        ...
        # if self.model_path.find('dynamic') != -1:
        ...
        #             self.sim.model.geom_size[self.sim.model.geom_name2id('object0'),1] = 0.025 + 0.02*(np.random.random()-0.5)
        #         if np.random.random() > 1: 
        #             if self.sim.data.sensordata[self.sim.model.sensor_name2id('r_finger_jnt')] > 0.03:
    ...
    def _set_action(self, action):
        ...
        action = np.concatenate([pos_ctrl_fnc(self.glob_obs), action]) if self.model_path.find('wall') == -1 else np.concatenate([pos_ctrl_fnc2(self.glob_obs), action])
...
#                self.broken_table = True
            if self.model_path.find('wall') == -1:
                ...
                # print(l_finger_force, r_finger_force)
                ...
                # if self.n_actions == 3:
                #     conc_stiffness_data = np.array([finger_force])/(self.max_stiffness * 2.0)
                # elif self.n_actions == 4:
                #     conc_stiffness_data = np.concatenate([[finger_force,
                #                          self.prev_stiffness]])/(self.max_stiffness * 2.0)
                # elif self.n_actions == 5:
                conc_stiffness_data = np.concatenate([[finger_force,
                                     self.prev_stiffness,
...
#        print("SITE POS: {}, SITE OFFSET: {}, GOAL: {}".format(self.sim.model.site_pos[site_id], sites_offset[site_id], self.goal[:3]))
    ...
    def _reset_sim(self):
        ...
        self.remaining_timestep = 50
        # self.sim.model.geom_size[self.sim.model.geom_name2id('object0'),1] = 0.025
