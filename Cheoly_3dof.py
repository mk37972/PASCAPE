...
while ep < num_ep:
    ...
    for i in range(lengh_ep):
        if np.linalg.norm(np.array([(a.goal[:2].reshape(-1,1)-obj_location)[0,0],(a.goal[:2].reshape(-1,1)-obj_location)[1,0]]))<0.005: # hold it still
            ...
            if render: print("STAGE4", a.est_torques_L[0,0], a.est_torques_R[0,0])
            ...
            start_grasp = i
            if render: print("STAGE1", a.est_torques_L[0,0], a.est_torques_R[0,0])
        elif a.est_torques_L[0,0] > -a.min_grip/10. and a.est_torques_R[0,0] < a.min_grip/10. and np.linalg.norm(obj_location - cen_location) < 0.01: # grasp the object
            obj_location = a.est_obj_pose.copy()
            # obj_location[1,0] += 0.005
            cen_location = a.des_p[1:3]
            ...
            episodeInfo.append(info)
            episodeObs.append(obsDataNew)
            if render: print("STAGE2", a.est_torques_L[0,0], a.est_torques_R[0,0])
        elif np.linalg.norm(np.array([(a.goal[:2].reshape(-1,1)-obj_location)[0,0],(a.goal[:2].reshape(-1,1)-obj_location)[1,0]]))>0.005: # manipulate the object
            cen_location = a.des_p[1:3]
            ...
            episodeInfo.append(info)
            episodeObs.append(obsDataNew)
            if render: print("STAGE3", a.est_torques_L[0,0], a.est_torques_R[0,0])
        if render: a.render()
...
if render != 1: 
    for i in [3,5]:
        fileName = "DarkManipulationComplDemo"
        if i == 3:
            fileName += "_3D"
        if i == 5:
            for j in range(num_ep):
                for k in range(lengh_ep):
                    observations[j][k]['observation'] = np.concatenate([observations[j][k]['observation'], [1., 1.]])
