...
class RolloutWorker:
    ...
    def generate_rollouts(self):
        ...
        # generate episodes
        obs, achieved_goals, acts, goals, successes, successes2, successes3, rews = [], [], [], [], [], [], [], []
        dones = []
        ...
        for t in range(self.T):
            ...
            try:
                ...
                self.venv.envs[0].env.sim.data.qvel[self.venv.envs[0].env.sim.model.joint_name2id('object:joint')] = 0.
                self.venv.envs[0].env.sim.data.qvel[self.venv.envs[0].env.sim.model.joint_name2id('object:joint')+1] = 0.
            ...
            if self.venv.envs[0].env.spec.id == 'FetchPickAndPlaceFragile-v1': # block
                ...
                success2 = (np.float32(self.venv.envs[0].env.prev_oforce).mean() < self.venv.envs[0].env.object_fragility)
...
#        print("--------------------New Rollout--------------------")
    ...
    def logs(self, prefix='worker'):
        ...
        if self.compute_Q:
            ...
            logs += [('mean_r', np.mean(self.r_history))]
