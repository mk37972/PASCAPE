...
class RolloutWorker:
    ...
    def __init__(self, policy, dims, logger, T, rollout_batch_size=1,
        ...
        self.R_j = np.matrix([[0.012,0],
                          [-0.012, 0.012]])
        self.R_j_inv = np.linalg.inv(self.R_j)
        self.R_j_L = np.matrix([[0.012,0],
                          [0.012, 0.012]])
        ...
        self.max_kj_R = np.transpose(self.R_j) * np.matrix([[2*self.Ksc1, 0],[0, 2*self.Ksc2]]) * self.R_j
        self.max_kj_L = np.transpose(self.R_j_L) * np.matrix([[2*self.Ksc1, 0],[0, 2*self.Ksc2]]) * self.R_j_L
    ...
    def reset_all_rollouts(self):
        ...
        with Session(bitfile="SCPC-lv-noFIFO_FPGATarget_FPGAmainepos_1XvgQEcJVeE.lvbitx", resource="rio://10.157.23.150/RIO0") as session:
            ...
            for i in range(25):
                ...
                des_mR = (np.transpose(self.R_j_inv)*des_tau_R / np.matrix([[2*self.Ksc1, 0],[0, 2*self.Ksc2]]) + self.R_j * Rj) / self.Rm 
                des_mL = (np.transpose(self.R_j_inv_L)*des_tau_L / np.matrix([[2*self.Ksc1, 0],[0, 2*self.Ksc2]]) + self.R_j_L * Lj) / self.Rm
    ...
    def generate_rollouts(self):
        ...
        with Session(bitfile="SCPC-lv-noFIFO_FPGATarget_FPGAmainepos_1XvgQEcJVeE.lvbitx", resource="rio://10.157.23.150/RIO0") as session:
            ...
            for t in range(self.T*3):
                ...
                max_kj_R = np.transpose(self.R_j) * np.matrix([[2*self.Ksc1, 0],[0, 2*self.Ksc2]]) * self.R_j
                max_kj_L = np.transpose(self.R_j_L) * np.matrix([[2*self.Ksc1, 0],[0, 2*self.Ksc2]]) * self.R_j_L
